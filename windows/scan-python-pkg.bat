@echo off
REM Windows Launcher for Python Package Scanner
REM Requires Git Bash or WSL to be installed

setlocal

set "SCRIPT_DIR=%~dp0"
set "BASH_SCRIPT=%SCRIPT_DIR%..\main_script\pkg_scan.sh"

REM Try to find bash (Git Bash first, then WSL)
where bash >nul 2>&1
if %ERRORLEVEL% EQU 0 (
    echo Starting Python Package Scanner...
    echo.
    bash "%BASH_SCRIPT%"
    goto :end
)
...
where wsl >nul 2>&1
if %ERRORLEVEL% EQU 0 (
    echo Starting Python Package Scanner via WSL...
    echo.
